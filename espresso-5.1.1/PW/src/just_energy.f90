...
!
! ... written by M. B. Goldey who just wanted an energy (from electrons.f90)
!
!----------------------------------------------------------------------------
SUBROUTINE just_energy()
  !----------------------------------------------------------------------------
  !
  ! ... General energy acquisition, also for hybrid functionals
  ...
  !
  !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  !%%%%%%%%%%%%%%%  Don't iterate hybrid functional  %%%%%%%%%%%%%%%%%%%%
  !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  !
  IF ( restart ) THEN
    write(*,*) "Don't restart- this needs a converged job to be meaningful"
  END IF
  !
  ! ... Main body. For hybrid functionals the exchange potential
  ! ... is calculated with the orbitals at previous step (none at first step)
  !
  CALL electrons_energy ( no_printout )
  !
...
9121 FORMAT(/'     scf convergence threshold =',1PE17.1,' Ry' )
  !
END SUBROUTINE just_energy
!
!----------------------------------------------------------------------------
SUBROUTINE electrons_energy ( no_printout )
  !----------------------------------------------------------------------------
  !
...
9085 FORMAT(/'     total all-electron energy =',0PF17.6,' Ry' )
  ...
  !
END SUBROUTINE electrons_energy
