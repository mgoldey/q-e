...
!  if (ionode) write(*,*) "x,y,z ",tau(1,na)*alat*.5292, tau(2,na)*alat*.5292, tau(3,na)*alat*.5292
...
SUBROUTINE one_atom_shifted_wfc (ik, wfcatom,iatom,nfuncs,idir,dx)
  ...
  !if (ionode) write(*,*) "idir ",idir
  !if (ionode) write(*,*) "dx ",dx
  ! alat in bohr, so 
  !if (ionode) write(*,*) "alat is ",alat
  ! 1 bohr= 0.529177 Angstrom
  ! 0.529177 bohr/ 1.0 angstrom 
  ...
  !if (ionode) write(*,*) "Atom ",iatom
  !if (ionode) write(*,*) "x,y,z ",x*alat*0.529177, y*alat*0.529177, z*alat*0.529177
...
!  if (ionode) write(*,*) "made it here" !fail
  ...
  call stop_clock ('atomic_wfc')
!  if (ionode) write(*,*) "exiting one_atom_shifted_wfc"
  return
