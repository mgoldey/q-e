...
SUBROUTINE epcdft_get_h
  ...
  !
  ! hc(1,1,s) = ecor1
  ! hc(1,2,s) = 0.5 ( (F1+F2) Sab - (Aa*Wab + Ab*Wba) )
  ! hc(2,1,s) = 0.5 ( (F2+F1) Sba - (Ab*Wba + Aa*Wab) )
  ! hc(2,2,s) = ecor2
  !
  ! Sab = Sab_up*Sab_down
  !
  ...
  USE epcdft_mod, ONLY : free1, free2, wmat, smat, hc, cor1, cor2, wamp1, wamp2
  USE klist,      ONLY : nks
  !
  IMPLICIT NONE
  !
...
!  DO s = 1, nks
    DO i = 1, 2
      DO j = 1, 2
        !
        wtot(i,j) = ( amps(i)*wmat(i,j,1) + amps(j)*wmat(j,i,1) ) * smat(i,j,2) + &
                    ( amps(i)*wmat(i,j,2) + amps(j)*wmat(j,i,2) ) * smat(i,j,1)
        !
        IF(i==j)THEN
          hc(i,j) = core(i)
        ELSE
